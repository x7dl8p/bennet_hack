      ...
      Papa.parse<any>(csvText, {
        ...
        complete: (results) => {
          if (results.errors.length > 0) {
            console.error(
              "[DEBUG ApiManager] CSV Parsing Errors:",
              results.errors
            );
          }

          const funds: MutualFundData[] = results.data
            .map((row: any, index: number) => {
              if (!row.scheme_name) {
                console.warn(
                  `[DEBUG ApiManager] Skipping row ${
                    index + 2
                  } due to missing scheme_name.`
                );
                return null;
              }
              ...
              return {
                ...
                nav: parseFloatSafe(row.nav) || 100.0,
          ...
          console.log(
            `[DEBUG ApiManager] Successfully parsed ${funds.length} funds from CSV.`
          );
...
class ApiManager {
  static async makeRequest(
    ...
    if (endpoint === "funds" && method === "GET") {
      console.log(
        "[DEBUG ApiManager] Intercepting /funds request, loading data from CSV."
      );
    ...
    if (endpoint.startsWith("search")) {
      console.log(
        `[DEBUG ApiManager] Intercepting ${endpoint} request, returning mock empty response.`
      );
      await new Promise((resolve) => setTimeout(resolve, 200));

      return Promise.resolve({
        message: "Mock AI response for query: " + queryParams?.q,
      });
    }

    console.warn(
      `[DEBUG ApiManager] No mock handler for ${method} ${endpoint}. Returning empty object.`
    );
    return Promise.resolve({});
  }
  ...
  static getFundResearchData = async (
    fundId: string,
    timeframe: string = "1y"
  ) => {
  ...
  static getFundPerformanceDetails = async (
    fundId: string,
    timeframe: string = "1y"
  ) => {
