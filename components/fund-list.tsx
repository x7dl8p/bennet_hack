...
export default function FundList({
  ...
  return (
    <div className="space-y-3">
      {funds.map((fund) => {
        // --- DEBUG LOG ---
        console.log(`[DEBUG FundList] Fund: ${fund.id}, Value for toFixed: ${fund.returns?.['1y'] ?? 0}, Type: ${typeof (fund.returns?.['1y'] ?? 0)}`);
        // --- END DEBUG LOG ---
        return (
          <Card
            key={fund.id}
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
               <div>
                 ...
                 <p className={cn(
                   "font-medium",
                   (fund.returns?.['1y'] ?? 0) > 0 ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"
                 )}>
                   {(fund.returns?.['1y'] ?? 0) > 0 ? "+" : ""}
               ...
               <div>
                 ...
                 <p className={cn(
                   "font-medium",
                   (fund.returns?.['3y'] ?? 0) > 0 ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"
                 )}>
                   {(fund.returns?.['3y'] ?? 0) > 0 ? "+" : ""}
               ...
               <div>
                 ...
                 <p className={cn(
                   "font-medium",
                   (fund.returns?.['5y'] ?? 0) > 0 ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"
                 )}>
                   {(fund.returns?.['5y'] ?? 0) > 0 ? "+" : ""}
